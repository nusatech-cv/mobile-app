...
class TopTherapistPage extends StatefulWidget {
  const TopTherapistPage({super.key});

  @override
  State<TopTherapistPage> createState() => _TopTherapistPageState();
}

class _TopTherapistPageState extends State<TopTherapistPage> {
  final TherapistController controller = Get.put(TherapistController());
  final HomeUserController homeUserController = Get.find();
            ...
            Expanded(
              child: Padding(
                ...
                child: (controller.isLoadingList.value ||
                    ...
                    : controller.topTherapistList.isEmpty
                        ? const Center(
                            child: Empty(
                        ...
                        : ListView.builder(
                            shrinkWrap: true,
                            itemCount: controller.topTherapistList.length,
                            itemBuilder: (context, index) {
                              return CardTherapist(
                                onTap: () => Get.to(() => TherapistPage(
                                    therapist:
                                        controller.topTherapistList[index])),
                                therapist: controller.topTherapistList[index],
                                isAppointment: false,
                              );
                            },
                          ),
                      ...
                      child: ListView(
                        children: [
                          ...
                          SingleChildScrollView(
                            scrollDirection: Axis.horizontal,
                            child: CustomTabBar(
                              titles: homeUserController.serviceList
                                  .map((element) => element.name!)
                                  .toList(),
                              selectedIndex: controller.tabBarSpeciality.value,
                              onTap: (index) =>
                                  controller.changeTabSpeciality(index),
                            ),
                          ),
                          16.0.height,
                          Text(
